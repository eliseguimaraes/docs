...
O behavior de Translate permite que você crie e recupere cópias traduzidas
de suas entidades em múltiplas línguas. Isso é feito utilizando-se tabelas ``i18n``
separadas, onde são armazenadas traduções para cada campo de qualquer tabela associada.

.. Alerta::

    Até esse momento o TranslateBehavior não dá suporte a chaves primárias compostas.

Um Tour Rápido
============

Após criar a tabela ``i18n`` em seu banco de dados anexe o behavior a qualquer tabela
que queira tornar traduzível:: 
...
Agora, selecione a língua a ser utilizada para recuperar as entidades mudando a língua da aplicação, o que vai afetar todas as traduções::

    // Em um controlador. Mude a língua para, por exemplo, espanhol
    I18n::locale('es');
    $this->loadModel('Articles');

Em seguida, recupere uma entidade existente::

    $article = $this->Articles->get(12);
    echo $article->title; // Imprime 'A title', ainda não traduzido
...
Agora experimente recuperar sua identidade novamente::

    $article = $this->Articles->get(12);
    echo $article->title; // Imprime 'Un Artículo', em espanhol


Trabalhar com múltiplas traduções pode ser feito utilizando-se um Trait especial em sua classe Entity::
...
É igualmente fácil salvar múltiplas traduções ao mesmo tempo::
...
Caso queira se aprofundar mais em como isso funciona ou em como ajustar o behavior às suas necessidades, continue lendo o restante desse capítulo.

Inicializando a tabela i18n
====================================

Para utilizar esse behavior, você deve criar uma tabela ``i18n`` com o schema correto. Atualmente, a única forma de carregar a tabela ``i18n`` é executando manualmente o seguinte script SQL em seu banco de dados:
...
Esse schema também está disponível como um arquivo sql em **/config/schema/i18n.sql**.

Uma observação em abreviações do nome da língua: o behavior de Translate não impõe nenhuma restrição ao identificador da língua, e os valores possíveis só são restringidos pelo tamanho/tipo da coluna ``locale``. ``locale`` é definida como ``varchar(6)``, caso se deseje utilizar abreviações como ``es-419`` (Espanhol para a América Latina, sgila com código de área `UN M.49
